...
# Mirrors real caller: w/x smoothed via si.smooth(tau2pole(0.02)), shift via
# normalGlidePole = tau2pole(0.008), snapping at attackEdge -- matching
# multitranspose.dsp's voiceOut()/process() smoothing chain exactly.
XPOSE_DSP = '''
import("stdfaust.lib");
xposeMaxDelay = 2000;
...
for i, f in enumerate(test_freqs):
    ...
    if i > 0:
        g_auto[start_b] = 0.0  # force attackEdge (gate 0->1) at each new segment
...
print("\n=== WITH REAL SMOOTHING CHAIN (matches multitranspose.dsp) ===")
for i, f in enumerate(test_freqs):
    start = i * seg_n
    end = start + seg_n
    steady = out[start + int(seg_n * 0.5):start + int(seg_n * 0.9)]
