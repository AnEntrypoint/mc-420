...
def measure_fft_peak(seg, sr=SAMPLE_RATE):
    n = len(seg)
    win = np.hanning(n)
...
print("=== FFT-based peak measurement (ZCR-independent) ===")
for i, f in enumerate(test_freqs):
    start = i * seg_n
    end = start + seg_n
    steady = out[end - int(1.0 * SAMPLE_RATE):end - 1000]
    measured = measure_fft_peak(steady)
