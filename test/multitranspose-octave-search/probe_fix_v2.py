...
# FIX v2: accumulate an UNWRAPPED cumulative sum of i (never re-scaled by w),
# then compute d = fmod(cum, w) purely for delay-line addressing at read time.
# This decouples the wrap RATE (governed only by i, cumulative, monotone)
# from w's instantaneous value -- a w change can only affect the read
# position's modulus, never how many "cycles" have actually elapsed.
XPOSE_DSP = '''
import("stdfaust.lib");
xposeMaxDelay = 2000;
...
print("\n=== FIX v2: unwrapped cumulative phase, w-independent rate ===")
for i, f in enumerate(test_freqs):
    start = i * seg_n
    end = start + seg_n
