...
xpose(w, x, s, sig) = de.fdelay(xposeMaxDelay,d,sig)*crossfadeGain +
    de.fdelay(xposeMaxDelay,d+w,sig)*(1-crossfadeGain), d, crossfadePos
with {
    i = 1 - pow(2, s/12);
    d = i : (+ : +(w) : fmod(_,w)) ~ _;
...
b = seg_n
print("d and crossfadePos across the 40Hz->440Hz boundary (samples b-5..b+2000, every 100):")
for i in range(b - 5, b + 3000, 100):
    print(f"  sample {i-b:+6d}: d={d_ch[i]:10.3f} cf={cf_ch[i]:.4f}")

print("\nd values just before boundary (last 40Hz w=960):", d_ch[b-10:b])
print("d values just after (first 440Hz w=109 samples):", d_ch[b:b+30])
