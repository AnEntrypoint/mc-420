...
# Gradually shift wTarget/xTarget target values (not a snap), matching a
# genuine live pitch-detection glide -- see if fmod's d-carryover NaNs even
# with smooth continuous change.
test_freqs = [40, 440]
seg_dur = 1.0
...
b = seg_n
print("output right after boundary (should glide smoothly if fine):")
print(out[b-5:b+300:20])
print("nan count total:", np.isnan(out).sum())
print("output range:", np.nanmin(out), np.nanmax(out))
