...
live mic signal (`dry`, bandpassed per-voice to the played note's own
frequency via `fi.resonbp(freqHz, 15.0, 1.0)` — a Q=15 bandpass, tightened
from an initial Q=2 after measurement showed Q=2 let too much
off-resonance broadband energy through, reading as "tinted mic passthrough"
rather than a real resonant instrument). There is NO synthetic exciter
anywhere (a held key with silent live input renders bit-exact silence) —
keys drive voice note/gate/vel only, the mic is the only sound source.
`modeFilterR`'s biquad has proper `(1-r)` numerator gain normalization
(unnormalized, it was permanently `ma.tanh`-saturating regardless of
input). `bwFloorT60 = 0.15` clamps every mode's minimum bandwidth
(measured/tuned jointly with the exciter's bandpass Q — the two parameters
compound and were re-derived together via a real tonal-vs-noise
spectral-energy-fraction metric, not loudness alone). `process()` order is
`(...) : *(outLevel) : ma.tanh` (outLevel drives INTO the limiter, not
scaled past it). `outLevel` default `25.0` (range `0.0..60.0`,
logarithmic-taper knob) — large because a real narrowband resonator only
captures a small fraction of a broadband/mistuned excitation's energy by
design; correct default was found by measuring realistic (off-pitch-tap)
excitation, not the unrealistically-favorable on-pitch case. `bassBoost`
gives the fundamental mode (mode 1 only) up to 1.35x gain below 220Hz,
tapering to unity at/above 220Hz (exact identity above the corner).
`aliasGuard` fades any mode whose real frequency crosses the top 5% of
Nyquist to silence rather than letting it fold back as a spurious alias
peak. `collision` (0..1, per-patch) is a bounded `ma.tanh`-based
waveshaper on each voice's own `bank()` output before summing — exact
identity at 0, adds real broadband grit/decay-tail energy at higher
values. Pitch-mod (`flexibility = max(0,min(1,(0.5-stretch)))`) adds a
small (~44 cents max), velocity- and stretch-scaled onset frequency bump
via a one-shot exponential decay envelope, retriggered on the same
...
`stretch`/`tone`/`level`/`collision`) glide via a `letrec`-based one-pole
that SNAPS to the true value on `ba.time==0` and glides only on later
changes (never a naive `si.smooth`, which zero-inits and fades in from
silence on every fresh DSP instance).
...
`collision` values are hand-set judgment calls, not swept. Knobs 5-6 are
direct performative dials (`applyResonodeDirectKnob`): tone brightness
(logarithmic taper — a linear Hz sweep compressed nearly all audible change
into the first 10-20% of the knob's travel) and level.
...
per-take offset. `recordStartMasterPhase` remembers the real, grid-deferred
recording-start instant purely as a RING INDEXING offset; `cycleOffset`
(accumulates `+masterLen` on each `masterPhase` wrap, reset at `armEdge`
only) restores which repetition of a multi-`masterLen` take is being read.
`wrapLen = roundedBeats * oneBeat` where `roundedBeats` is a plain
nearest-whole-beat rounding of the take's own recorded length (the C++
quantizer already produces a clean power-of-2 multiple of `masterLen`).
Content is never trimmed/discarded — recording start is deferred to the
next fine grid tick (`masterLen/16`) via `armPending`/`gridTickCrossed`, so
the take's real content always begins on-grid without losing any captured
audio.
