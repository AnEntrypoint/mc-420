...
**Fixed by holding `effDetNote` at its own last-settled (pre-onset) value
during the untrust window, not blending it toward `targetNote`.**
`heldDetNote` latches `detNote`'s value one sample before the gate's
rising edge (`ba.if(rising, detNote:mem, prev) ~ _`) and holds it through
untrust; `effDetNote = detNote*(1-untrust) + heldDetNote*untrust`. This
freezes whatever pitch-lock relationship was already established
(correctly re-locking within single-digit cents by 5-10ms for a voice
retriggered while the tracker/lock context is warm — the common case of
rapid melodic/chordal playing) instead of forcing the shift to zero. For a
genuinely cold first note (silence before onset, no prior lock to freeze
onto) this is dramatically better than BOTH the pre-`onsetUntrust` file
(worst-case -2621 to -2987 cents at 5-10ms, i.e. up to ~2.5 octaves flat,
converging by 150-200ms) and the blend-to-unison version it replaces
(worst-case ~350-530 cents but at the cost of zeroing the lock on every
subsequent warm retrigger too) — held-last-shift's worst cold-start case
measures ~320 cents at 10ms, converging the same ~150-200ms, while adding
zero regression on the warm-retrigger case the blend-to-unison version
broke. True disabled-state check (every voice's gate held at 0 the entire
render): bit-exact silent output, unaffected by this change (gated
...
original `onsetUntrust` fix it replaces the blend-target of.
