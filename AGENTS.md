...
genuine (if slow — 100-500+ real seconds per iteration on this dev
host's hardware) local reproduction, so further bisection no longer
strictly requires CI round-trips (~15-20 min each) — but it does require
budgeting real per-iteration cost (this session's own attempts took
100-500+ seconds each and the fastest useful signal came from watching
whether a run cleared or blew past the working baseline's own ~113s
mark, not from waiting for full completion). The real open question is
now much narrower: WHY does an isolated, non-recursive, purely additive
`hslider` declaration (`extFreqDet = hslider("fx/extfreqdet", 220.0,
minTrackHz, maxTrackHz, 0.01);`) reproducibly destabilize this specific
file's real Faust codegen path, when the exact same declaration compiles
trivially fast in `resonode_synth.dsp`/`guitar_lofi_fx.dsp`? A future
session should use the local `boxToSource` method to test candidates
narrower than "add one hslider" (e.g., does merely ADDING the hslider
declaration with ZERO other changes — not even referencing it anywhere —
reproduce the regression, mirroring the `periodicityConfidenceAt`
defined-but-unused precedent documented above for a different symptom in
this same file?) before concluding anything about the specific mechanism.
