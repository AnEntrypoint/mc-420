...
byte-identical `f9644be` (confirmed via `diff`, zero output) — the
original zero-crossing `trackPitchHzAndHp`/`octaveCorrect`/`jumpGuard`
tracker, `jumpGuard`-only baseline, plosive symptom still fully open.
`effects/pitchtracker-src/pitchtracker_ac.dsp` (the verified-working
autocorrelation tracker, relocated per step 4 above),
`.github/workflows/build-lv2.yml`'s `pitchtracker-lv2` job, and
`audio_thread.cpp`'s inert `Lv2Host pitchTrackerFx`/`pitchTrackerBuf`
plumbing are all left in place as real, CI-verified infrastructure — the
LV2 bundle genuinely builds and would genuinely run if
`multitranspose.dsp` read its output, it's just not wired in. The concrete
next step for a future session: get real local `faust`/Docker access,
re-apply the `fx/extfreqdet` hslider change to `multitranspose.dsp` alone
(the smallest possible reintroduction, matching the already-proven-safe
control-rate mechanism from step 3), and measure real local
`faust -lang cpp -I dsp -I effects/home/faust dsp/aloop.dsp` compile time
directly and repeatably — bisecting locally in seconds per iteration
instead of ~15-20 minutes per CI round-trip.
