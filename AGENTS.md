...
- `shiftAmount` was documented here as "sampled once per voice at
  ...
  `shiftStep` glided toward `shiftTarget` EVERY SAMPLE for the whole note,
  and `shiftTarget = targetNote - heldDetNote` with `heldDetNote` still
  following the detected input pitch. So the locked output tracked whatever
  the input did: on a +-30 cent input wobble (ordinary vibrato or decay
  drift) the shift target swings 58 cents. `heldDetNote` and `shiftStep`
  now both freeze once the `inLockWarmup` window closes.
- `trackingAllowed` was `(trustedTracker > 0.5) | inLockWarmup`, so during
  warmup the detected pitch was taken WHETHER OR NOT the tracker was
  trusted. `pitchtracker.lv2` deliberately holds its output at 0.0 for the
  first ~35ms of any fresh onset, which makes `extFreqDetApplies` false and
  sends `freqDet` to the internal zero-crossing fallback — the tracker this
  same section documents as taking >400ms to converge and drifting
  non-monotonically. Every fresh note was therefore seeded from it. Now
  `trackingAllowed = trustedTracker > 0.5`, so the lock waits for the real
  tracker instead of latching onto the fallback's worst reading.

Failure mode of the fix is strictly better than what it replaced: if the
tracker never becomes trusted inside the 80ms window, `heldDetNote` holds
`lastConvergedNote`, seeded at `attackEdge` from the previous converged
note — stale but plausible, rather than actively wrong. The tracker's own
spec is ~35ms against an 80ms window, so there is 45ms of margin.

**The general lesson, and it is the expensive one: this file's prose was
right and the code had drifted from it.** The "sampled once at attackEdge"
sentence had been sitting here describing behaviour the code stopped
having. That is exactly the failure this document's own working rule
...
`ffunction`, so the JIT limitation documented elsewhere in this file does
not apply to it) against all 16 real instrument recordings, cross-checked
against an FFT harmonic-comb score rather than a single autocorrelation
reference:

  accurate within 50 cents   9 files, spanning 66Hz to 706Hz
  genuinely wrong            3 files, ALL at or above 500Hz
  reference/range artefacts  4 files

The three real failures are `oboe_mid_C5B5` (reads 262.9Hz against a true
527Hz — an exact octave-down subharmonic lock, comb score 296.9 for 527 vs
214.9 for 263), `violin_high_sulE_C7E7` (reads 436.4Hz against ~1035Hz,
comb score 9.6 vs 173 — not even a subharmonic, just a bad lock), and
`trumpet_high_C6` (-63.9 cents). `piano_low_A1` correctly returns 0: its
f0 is ~55Hz, below the tracker's own 60Hz `minTrackHz` floor, so the
no-lock is right rather than a failure.

`subharmonicPromote` exists for exactly the oboe case and does not catch
it, and the reason is structural rather than a mistuned constant: it
requires `c2 > cCoarse + subharmDominanceMargin`, but at a TRUE subharmonic
lag the normalised autocorrelation is genuinely higher than at the
fundamental on a periodic signal, so that test can never fire for the
octave-down case it was meant to fix. `subharmDominanceMargin` is only
0.004, so widening it is not the answer.

This is deliberately NOT fixed. Below 500Hz — which is guitar, bass, and
both vocal ranges, i.e. everything this pitch-lock is played with — the
tracker is accurate everywhere measured (-3.7c bassoon, -6.1c cello, -6.6c
violin low, +6.3c trumpet low, -12.7c piano mid, -2.5c marimba mid, -3.6c
vibraphone, +6.6c piano high, +22.9c vocal). The failures are confined to a
register the effect is not used in, and `pitchtracker_ac.dsp` is on the
wrong side of this file's own compile-time-cliff warning — its own
description records a multi-GB codegen blowup from one attempted change to
per-candidate tau selection. Re-opening this needs a real measured win that
justifies that risk.
...
- **The crossfade LENGTH is divided by the pitch ratio on upward shifts,
  and that division is what makes the correlation assumption above true.**
  ...
  sample. Left undivided, the readers travel `per*scale` during the fade and
  drift apart by `(scale-1)` periods before it ends, so by the end they are
  NOT one period apart and the equal-gain fade is summing two uncorrelated
  reads. That was a real, reported microclick: measured at 40x the signal's
  own median slew rate, appearing at +10 semitones (exactly where the drift
  crosses three quarters of a period) and worsening monotonically to +14,
  clean at +9 and below, concentrated at low input pitches where the period
  is long. Instrumenting the engine put 30 of 30 clicks adjacent to a splice
  and none away from one. Dividing by the ratio keeps reader travel at one
  period regardless of shift: clicks go to zero across 112 synthetic cases
  and the whole 70-200Hz boundary sweep. The divisor is clamped to never go
  below 1.0, so downshift and unity keep their original fade length and are
  bit-identical over 40 cases — the mono pedal-ride path is untouched.
  Onset latency is unchanged at 1.35-1.40ms across every pitch and shift.

**Splice-parameter sweeps, all measured against the real instrument corpus
using degradation ratio (output roughness / that file's own input
roughness, which is the metric that does not punish breathy sources for
being breathy — the vocal files score 0.34-0.45 roughness at the INPUT and
the engine only adds ~7%, whereas violin input is 0.017 and the shipped
engine took it to 0.106).** Mean degradation over 16 instruments fell from
**2.82 to 2.21** with the crossfade fix, worst cases improving most: violin
low 6.10 -> 3.03, violin high 4.78 -> 2.83, cello 4.06 -> 2.70. Four
further knobs were swept and every one of them is ALREADY at its optimum,
so do not re-tune them without new evidence:

- `m_respliceFrac` 1.0 is best (2.215) against 1.5 (3.176) and 2.0 (2.996).
  Letting drift accumulate before correcting is clearly worse.
- `SINC_TAPS` 16 vs 32 is a dead heat (2.215 vs 2.216). Doubling the kernel
  doubles the interpolation cost for nothing; the residual is splice
  quality, not interpolation quality.
- The splice candidate search width of +-3 is best (2.215) against +-6
  (2.223) and +-10 (2.249) — a wider search finds candidates that match on
  value but sit further from the ideal period.
- `m_transientHold` at 2 periods is neutral (2.215 / 2.217 / 2.216 for
  2 / 1 / 0 periods), so it is kept for the double-played-attack protection
  it was added for.
