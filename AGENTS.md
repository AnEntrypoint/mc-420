...
ramp. The FIRST attempt at this redesign was itself WITNESSED as "too small,
not really shuffling much" — its step tables were sparse (long flat runs of
`0` between occasional nonzero entries, e.g. one nonzero step in every 3-4)
and its clamp bound was tight (`±8` steps), so most of the 16-step cycle
produced no audible offset at all. Fixed by making every table entry nonzero
(no flat stretches) and doubling the clamp headroom to `±16` steps, so a
single held button alone is clearly audible (measured ~70-90ms of real
swing at the recorded tempo used for verification) and combinations scale up
from there rather than being crushed by the clamp. Each of the 4 buttons
selects a distinct fixed 16-entry step table (`kShuffleStep0`: classic
swing, alternating steps delayed hard; `kShuffleStep1`: dotted/triplet
gallop, every 3rd step pulled back hard against a slight forward push on the
rest; `kShuffleStep2`: push/drag, alternating ahead/behind on EVERY 16th
note, the sharpest single-button amount; `kShuffleStep3`: broken/polymetric,
a 5-step-over-16 displacement moving every single step), each entry a fixed
offset in units of `stepLen/16` (one 16th-note's worth of samples divided
into 16 sub-steps). `shuffleOffsetSamples` looks up the CURRENT 16th-note
step from `phaseNorm` and sums the step values of every currently-held
button (`shuffleGain[b] > 0.0f`, already an instant 0/1 snap per the
button-fade fix
above, not a partial blend) — the result changes ONLY at step boundaries and
is applied with zero interpolation, clamped to `±kShuffleClampSteps` (16) to
